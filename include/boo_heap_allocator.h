					...
					, const ::booldog::debug::info& debuginfo = debuginfo_macros )
				{
					...
					if( begin )
					{
						if( total > pointertotal )
							::booldog::interlocked::exchange_add( &_size_of_allocated_memory , total - pointertotal );
						else
							::booldog::interlocked::exchange_subtract( &_size_of_allocated_memory , pointertotal - total );
						size_t begin_size = total - size;
						size_t pointerinfosize = pointertotal - pointersize;
						if( pointerinfosize != begin_size )
							memmove( &begin[ begin_size ] , &begin[ pointerinfosize ] , pointersize );
					...
					::booldog::interlocked::exchange_subtract( &_size_of_allocated_memory , pointertotal );
					return 0;
				};
			public:
				...
				virtual void free( void* pointer )
				{
					...
					{
						::free( ptr );
						::booldog::interlocked::exchange_subtract( &_size_of_allocated_memory , offsize );
					}
					else
						throw int();
				};
