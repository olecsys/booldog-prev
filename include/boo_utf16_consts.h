#ifndef BOO_UTF16_CONSTS_H
#define BOO_UTF16_CONSTS_H
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
#include "boo_types.h"
namespace booldog
{
	namespace consts
	{
		namespace utf16
		{
			const ::booldog::byte _utf16_to_cp1251_page_from_152_to_187[] = 
			{
				0x98 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0x00 , 0xa0 , 0x00 , 0x00 , 0x00 , 0xa4 , 0x00 , 0xa6 , 
				0xa7 , 0x00 , 0xa9 , 0x00 , 0xab , 0xac , 0xad , 0xae , 0x00 , 0xb0 , 0xb1 , 0x00 , 0x00 , 0x00 , 0xb5 , 
				0xb6 , 0xb7 , 0x00 , 0x00 , 0x00 , 0xbb
			};
			const ::booldog::byte _utf16_to_cp1251_page_from_1025_to_1169[] = 
			{
				
			};
			const ::booldog::byte _utf16_to_cp1251_page_from_8211_to_8482[] = 
			{
				
			};
		};
	};
};
#endif
