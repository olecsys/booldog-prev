...
namespace booldog {
  ...
  private:
    ...
    result_file(const ::booldog::result_file&) : result() {}
    ::booldog::result_file& operator=(const ::booldog::result_file&) { return *this; }

  public:
    ::booldog::io::file* file;
    result_file(void) { file = 0; }
  ...
  namespace io {
    ...
    private:
      ...
      ::booldog::io::file& operator=(const ::booldog::io::file&) { return *this; }

    public:
      static bool mbsopen(::booldog::result_file* pres, booldog::allocator* allocator,
                          const char* name_or_path, int file_mode,
        ...
        booldog::named_param settings[] = {
          BOO_SEARCH_NAMED_PARAM_PPARAM("search_paths"), BOO_SEARCH_NAMED_PARAM_PCHAR("root_path"),
        ...
        if(resbool.bres) {
          ...
          if(res_root_dir.mbchar) {
            if(::booldog::utils::io::path::mbs::normalize(
                 &resres, res_root_dir.mbchar, res_root_dir.mblen, res_root_dir.mbsize) == false) {
              res->copy(resres);
              goto goto_return;
            }
            ...
            for(size_t index = 0;
              ...
              if(settings[0].pparamvalue[index].type == ::booldog::enums::param::type_pchar) {
                if(::booldog::utils::string::mbs::insert<0>(
                     ...
                     settings[0].pparamvalue[index].pcharvalue, 0, SIZE_MAX, debuginfo) == false) {
                  res->copy(resres);
                  goto goto_return;
                }
                ...
                if(::booldog::utils::string::wcs::insert(
                     &resres, allocator, false, res_root_dir.mblen, res_root_dir.mbchar,
                     res_root_dir.mblen, res_root_dir.mbsize,
                     settings[0].pparamvalue[index].pwcharvalue, 0, SIZE_MAX, debuginfo) == false) {
                  res->copy(resres);
                  goto goto_return;
                }
              } else
      ...
      static bool mbsopen(::booldog::result_file* pres, booldog::allocator* allocator,
                          const char* name_or_path, int file_mode, const char* search_path,
                          bool exedir_as_root_path,
                          const ::booldog::debug::info& debuginfo = debuginfo_macros) {
        debuginfo = debuginfo;
        booldog::param search_paths_params[] = {BOOPARAM_PCHAR(search_path), BOOPARAM_NONE};
        booldog::named_param load_params[] = {
          BOONAMED_PARAM_PPARAM("search_paths", search_paths_params),
          BOONAMED_PARAM_BOOL("exedir_as_root_path", exedir_as_root_path), BOONAMED_PARAM_NONE};
        return ::booldog::io::file::mbsopen(pres, allocator, name_or_path, file_mode, load_params);
      }
      bool close(::booldog::result* pres,
                 const ::booldog::debug::info& debuginfo = debuginfo_macros) {
        debuginfo = debuginfo;
      ...
      bool write(::booldog::result* pres, ::booldog::byte* buffer, size_t buffer_size,
                 size_t portion_size, size_t& written,
                 const ::booldog::debug::info& debuginfo = debuginfo_macros) {
        debuginfo = debuginfo;
        ::booldog::result locres;
        BOOINIT_RESULT(::booldog::result);
      ...
      bool read(::booldog::result_buffer* pres, booldog::allocator* allocator, size_t portionsize,
                const ::booldog::debug::info& debuginfo = debuginfo_macros) {
        ...
        if(res->bufsize < portionsize) {
          ...
          res->buf =
            res->allocator->realloc_array<unsigned char>(res->buf, res->bufsize, debuginfo);
          if(res->buf == 0) {
            res->booerr(::booldog::enums::result::booerr_type_cannot_alloc_memory);
            goto goto_return;
          }
      ...
      bool position(::booldog::result_int64* pres,
                    const ::booldog::debug::info& debuginfo = debuginfo_macros) {
        debuginfo = debuginfo;
      ...
      bool position(::booldog::result* pres, ::booldog::int64 offset,
                    ::booldog::enums::io::file_position_origin origin,
                    const ::booldog::debug::info& debuginfo = debuginfo_macros) {
        debuginfo = debuginfo;
        ::booldog::result locres;
        BOOINIT_RESULT(::booldog::result);
      ...
      bool flush(::booldog::result* pres,
                 const ::booldog::debug::info& debuginfo = debuginfo_macros) {
        debuginfo = debuginfo;
        ::booldog::result locres;
        BOOINIT_RESULT(::booldog::result);
#ifdef __WINDOWS__
#ifdef __x64__
        ::booldog::int64 temp = (::booldog::int64)_file;
        HANDLE file_handle = (HANDLE)temp;
#else
        HANDLE file_handle = (HANDLE)_file;
#endif
       ...
       * Every next function call read next file line. If @param pres bufdatasize member is equal 0
       * then file reached the end.
       * @param pres store the function result or detailed error
       * @param debuginfo a debug information
       * @return The function result
       */
      template <size_t step>
      bool readline(::booldog::results::buffer& pres,
                    const ::booldog::debug::info& debuginfo = debuginfo_macros) {
        pres.clear();
        ...
        for(;;) {
          if(pres.bufsize == pres.bufdatasize) {
            pres.bufsize += step;
            pres.buf =
              pres.allocator->realloc_array<unsigned char>(pres.buf, pres.bufsize, debuginfo);
            if(pres.buf == 0) {
          ...
          ::booldog::utils::string::mbs::indexof<1, 2>(resindexof, pres, false, (char*)pres.buf,
                                                       offset, pres.bufdatasize - offset, search, 2,
                                                       debuginfo);
...
#ifdef __WINDOWS__
          readres = _read(
            _file, &res->buf[res->bufdatasize], (unsigned int)(res->bufsize - res->bufdatasize));
#else
